 ...
 * @param err Error object to be included in the response.
 */
export const makeResponsesException = (res: Response, err: Error) => {
  const msg = {
    OK: 0,
    Message: err,
  };
 ...
 */
export const makeResponsesOk = (res: Response, code?: 'Success' | 'UnexpectedError' | 'Error') => {
  const msg = {
    OK: 1,
 ...
 * @param code Optional code parameter to specify the success message. Defaults to "Success".
 */
export const makeResponsesOkData = (res: Response, data: any, code?: 'Success' | 'UnexpectedError' | 'Error') => {
  const msg = {
    OK: 1,
    Data: data,
    Message: messages[code ?? 'Success'],
